...
;;; Setup Procedures ;;;
;;;;;;;;;;;;;;;;;;;;;;;;
extensions [nw] ;; tool for easy calculations
globals [char-path-len] ;; for global access, might be a better solution
...
to setup
  ...
  reset-ticks
  set char-path-len nw:mean-path-length
end
...
to go
  ...
  if layout? [ layout ]
  set char-path-len nw:mean-path-length ;; calculations for the characteristic path length
  tick
end
