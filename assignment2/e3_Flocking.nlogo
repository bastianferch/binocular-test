turtles-own [
  ...
  obstacles          ;; agentset of nearby obstacles
  nearest-neighbor   ;; closest one of our flockmates
  nearest-obstacle   ;; closest obstacle
]
...
to go
  ...
  repeat 5 [ ask turtles [ fd 0.2 ] display
    ; kills turtles on red spots
    ask patches with [pcolor = red][
      ask turtles-here [
        set current-pop (current-pop - 1)
        die
      ]
    ]
  ]
...
to create-world ;; observer procedure
  ...
  repeat obstacle-amount[
    let random-x random(world-width)
    let random-y random(world-height)
    ; print word random-x random-y
    ask patches[
      if (distancexy random-x random-y ) < obstacle-radius[
        set pcolor red
      ]
    ]
  ]
...
to find-obstacles ;; turtle procedure, find obstacles
  set obstacles  patches in-radius (vision * 3) with [pcolor = red]
end

to find-nearest-obstacle ;; turtle procedure, obstacle avoidance
  set nearest-obstacle min-one-of obstacles [distance myself]
end

;;; AVOID

to avoid
  turn-away (towards nearest-obstacle) max-separate-turn
end
...
"" ""
PENS
"default" 0.3 0 -13840069 true "" "plot count turtles"
